-- Promotes operator changes to a first-class standalone unit process.
-- The existing driver_change event becomes the process start so historical
-- records keep their identity. driver_change_finished closes the process.
  ...
  check (
    behavior = any (
      array[
        ...
        'fuel_start'::text,
        'fuel_end'::text,
        'driver_change_start'::text,
        'driver_change_end'::text,
...
begin
  if exists (
    ...
    where event_type in ('driver_change', 'driver_change_finished')
      and is_system = false
  ) then
    raise exception
      'Existen estatus personalizados con los códigos driver_change o driver_change_finished. Elimínalos antes de ejecutar esta migración.';
  end if;
end;
$$;

-- The current validation trigger intentionally protects system behavior from
-- manual edits. Disable it only while this migration upgrades driver_change
-- from a standalone status to a system process.
drop trigger if exists validate_unit_event_type_setting_change_trigger
...
-- Consolidate standalone system-process seeding for future projects.
drop trigger if exists seed_diesel_refueling_event_settings_trigger
  on public.projects;

drop function if exists public.seed_diesel_refueling_event_settings();

create or replace function public.seed_standalone_system_event_settings()
returns trigger
language plpgsql
security definer
...
create trigger seed_standalone_system_event_settings_trigger
after insert on public.projects
for each row
execute function public.seed_standalone_system_event_settings();

create or replace function public.is_unit_standalone_driver_change_active(
  target_unit_id uuid
)
returns boolean
language sql
...
as $$
  select coalesce(
    (
      select event_type.behavior = 'driver_change_start'
...
revoke all on function public.is_unit_standalone_driver_change_active(uuid)
  from public;
grant all on function public.is_unit_standalone_driver_change_active(uuid)
  to anon;
grant all on function public.is_unit_standalone_driver_change_active(uuid)
  to authenticated;
grant all on function public.is_unit_standalone_driver_change_active(uuid)
...
begin
  ...
  if new.unit_movement_id is null then
    ...
    if configured_event_type.behavior = 'driver_change_start'
       and public.is_unit_standalone_driver_change_active(new.unit_id) then
      raise exception
        'La unidad ya tiene un cambio de operador activo.';
    end if;

    if configured_event_type.behavior = 'driver_change_end'
       and not public.is_unit_standalone_driver_change_active(new.unit_id) then
      raise exception
        'La unidad no tiene un cambio de operador activo.';
    end if;
    ...
    if configured_event_type.behavior <> 'driver_change_end'
       and public.is_unit_standalone_driver_change_active(new.unit_id) then
      raise exception
        'Finaliza el cambio de operador antes de registrar otro estatus.';
    end if;
  else
    select
      movement.unit_id,
...
create trigger validate_unit_event_type_setting_change_trigger
before update on public.unit_movement_event_action_settings
for each row
execute function public.validate_unit_event_type_setting_change();
...
with check (
  ...
  and not public.is_unit_standalone_driver_change_active(unit_id)
  and exists (
    select 1
    from public.shifts shift
